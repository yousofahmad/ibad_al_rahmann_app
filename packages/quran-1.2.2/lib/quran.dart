...
import 'package:quran/translations/en_clear_quran.dart';
...
String getSurahNameArabicTashkel(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surahArabicTashkel[surahNumber - 1];
}
...
String getVerseTranslation(
  ...
  switch (translation) {
    case Translation.enClearQuran:
      return enClearQuran.firstWhere((element) =>
          element['surah_number'] == surahNumber &&
          element['verse_number'] == verseNumber)['content'];
    case Translation.frHamidullah:
      return frHamidullah.firstWhere((element) =>
          element['surah_number'] == surahNumber &&
          element['verse_number'] == verseNumber)['content'];
  }
  // var translationText = Translation.enSaheeh;
...
Map searchWords(String words) {
  List<Map> result = [];
  for (var i in quranNormalText) {
    if (i['content'].contains(words)) {
      result.add({
...
String removeVerseTashkel(String verse) {
  return DartArabic.stripTashkeel(DartArabic.stripDiacritics(verse));
}
