...
class _NewKhatmaScreenState extends State<NewKhatmaScreen> {
  ...
  String _noneDivision =
      'daily'; // sub-choice when _reminderType == 'none': daily or prayer
  ...
  /// Whether the division is prayer-based (5 wirds/day)
  bool get _isPerPrayer =>
      _reminderType == 'prayer' ||
      (_reminderType == 'none' && _noneDivision == 'prayer');

  /// The actual division type to store (daily or prayer)
  String get _effectiveDivision =>
      _reminderType == 'none' ? _noneDivision : _reminderType;

  /// Pages per day, dynamically computed based on mode and division type
  ...
  int get _estimatedWirds {
    return _isPerPrayer ? _estimatedDays * 5 : _estimatedDays;
  }
  ...
  void _startKhatma() async {
    ...
    await khatmaCubit.startNewKhatma(
      ...
      notificationType: _effectiveDivision,
      enableNotifications: _reminderType != 'none',
  ...
  Widget build(BuildContext context) {
    ...
    if (_isPerPrayer && !_canDistributeOverPrayers) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          setState(() {
            if (_reminderType == 'prayer') _reminderType = 'daily';
            if (_reminderType == 'none' && _noneDivision == 'prayer') {
              _noneDivision = 'daily';
            }
          });
        }
                  ...
                  if (!_isByAmount)
                    ...
                    Column(
                      children: [
                        ...
                        Text(
                          _isPerPrayer ? "بعد كل صلاة (×٥ يومياً)" : "يومياً",
                          style: TextStyle(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                child: GestureDetector(
                                  onTap: () =>
                                      setState(() => _noneDivision = 'daily'),
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(
                                      vertical: 8,
                                    ),
                                    decoration: BoxDecoration(
                                      color: _noneDivision == 'daily'
                                          ? goldColor
                                          : Colors.transparent,
                                      borderRadius: BorderRadius.circular(12),
                                    ),
                              ...
                              Expanded(
                                child: GestureDetector(
                                  onTap: () {
                                    if (_canDistributeOverPrayers) {
                                      setState(() => _noneDivision = 'prayer');
                                    }
                                  },
                                  child: Container(
                                    padding: const EdgeInsets.symmetric(
                                    ...
                                    child: Center(
                                      child: Text(
                                        "بعد كل صلاة",
                                        style: TextStyle(
                                          color: !_canDistributeOverPrayers
                                              ? Colors.grey
                                              : (_noneDivision == 'prayer'
                                                    ? Colors.black
                                                    : goldColor),
                                          fontWeight: FontWeight.bold,
                                          fontFamily: AppConsts.cairo,
        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle: TextStyle(color: goldColor.withValues(alpha: 0.3)),
            ),
            onSubmitted: (text) {
              ...
              onPressed: () {
                ...
                if (parsed != null && parsed >= min && parsed <= max) {
                  ...
                  ScaffoldMessenger.of(this.context).showSnackBar(
                    SnackBar(
                      content: Text("الرجاء إدخال رقم بين $min و $max"),
