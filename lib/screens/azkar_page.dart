...
class _AzkarPageState extends State<AzkarPage> {
  ...
  /// Returns a session-scoped key that resets based on prayer times.
  /// Morning Azkar resets at Maghrib. Evening Azkar resets at Fajr.
  Future<String> _getSessionKey(String base) async {
    ...
    try {
      ...
      if (widget.jsonFile.contains('morning')) {
        final maghrib = prayers.firstWhere((p) => p.id == 'maghrib').time;
        // If it's past Maghrib, we are technically preparing for tomorrow's morning cycle
        if (now.isAfter(maghrib)) {
          cycleDate = now.add(const Duration(days: 1));
        }
      } else if (widget.jsonFile.contains('evening')) {
        final fajr = prayers.firstWhere((p) => p.id == 'fajr').time;
        // If it's before Fajr (very early morning), we are still in yesterday's evening cycle
        if (now.isBefore(fajr)) {
          cycleDate = now.subtract(const Duration(days: 1));
        }
        // Note: After Fajr, the key becomes "today", resetting it for today's coming evening.
      }
    } catch (e) {
      debugPrint(
        "AzkarPage: Prayer-based key failed, falling back to date: $e",
      );
    }

    final dateStr = DateFormat('yyyy-MM-dd').format(cycleDate);
    return '${base}_${widget.jsonFile}_$dateStr';
  }
          ...
          child: _isLoading
              ? Center(
                  child: Padding(
                    ...
                    child: AppSkeleton.card(height: 300.h),
                  ),
                )
              : Column(
                  children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      icon: Icon(Icons.share, color: _goldColor, size: 26.sp),
