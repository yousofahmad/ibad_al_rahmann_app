...
class _MuezzinSelectionScreenState extends State<MuezzinSelectionScreen> {
  ...
  Future<void> _fetchRemoteData() async {
    try {
      final response = await _dio.get("${_baseUrl}app_config.json");
      if (response.statusCode == 200) {
        final rawData = response.data is String
            ? jsonDecode(response.data)
            : response.data;

        List<dynamic> remoteMuezzins;
        if (rawData is List) {
          // If the JSON is a direct list of categories
          remoteMuezzins = rawData;
        } else if (rawData is Map && rawData.containsKey('muezzins')) {
          // If the JSON is an object with a 'muezzins' key
          remoteMuezzins = rawData['muezzins'];
        } else {
          return;
        }
        ...
        if (mounted) {
          setState(() {
            muezzinData.clear();
            muezzinData.addAll(List<Map<String, dynamic>>.from(remoteMuezzins));
            _filteredData = List.from(muezzinData);
          });
  ...
  Future<void> _downloadFile(String id) async {
    ...
    try {
      // Look into audio/adhan/ folder
      final url = "${_baseUrl}audio/adhan/${id.trim()}.mp3";
  ...
  Future<void> _playPreview(String id) async {
    ...
    try {
      ...
      if (id == 'nafis') {
        ...
        const packageName = 'app.ibad_al_rahmann';
        ...
        // Not local: attempt streaming from URL (audio/adhan/ subfolder)
        try {
          final url = "${_baseUrl}audio/adhan/${id.trim()}.mp3";
