...
class _PrayerDetailModalState extends State<PrayerDetailModal> {
  int _adhanOffset = 0;
  int _iqamaDelay = 15;
  String _adhanMode = 'sound'; // 'sound' | 'silent_notif' | 'none'
  String _iqamaMode = 'none'; // 'sound' | 'silent_notif' | 'none'
  ...
  Future<void> _loadIqamaSettings() async {
    ...
    setState(() {
      ...
      _iqamaMode =
          prefs.getString('iqama_mode_$key') ?? (iqamaBool ? 'sound' : 'none');
    });
  }
  ...
  Future<void> _loadNotifStatus() async {
    ...
    setState(() {
      _adhanMode =
          prefs.getString('adhan_mode_$key') ?? (legacyBool ? 'sound' : 'none');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final containerColor = isDark ? const Color(0xFF000000) : Colors.white;
  ...
  /// Events that use fixed custom voice-over files in res/raw — no adhan picker.
  bool _isCustomSoundEvent() {
    const customSoundIds = {
      'sunrise',
      'duha',
      'first_third',
      'midnight',
    ...
    return customSoundIds.contains(widget.prayer.id.toLowerCase());
  }
  ...
  /// Compact 3-icon mode selector: 🔔 sound | 🔕 silent | ✖ none
  Widget _buildModeSelector(String current, ValueChanged<String> onChanged) {
    final modes = [
      ('sound', Icons.volume_up, const Color(0xFFD0A871)),
      ('silent_notif', Icons.notifications_outlined, Colors.blueGrey),
      ('none', Icons.block_outlined, Colors.red),
    ];
    return Row(
      mainAxisSize: MainAxisSize.min,
